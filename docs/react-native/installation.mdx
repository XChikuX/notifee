...
Notifee now ships an official Expo config plugin. If you use Expo with `expo prebuild`, prefer the plugin for native build-time tasks such as:

- creating and maintaining an iOS Notification Service Extension,
- adding APNs entitlements and iOS background modes,
- enabling communication-notification capabilities,
- generating or copying Android notification icon resources, and
- wiring the iOS extension target into EAS app extension config.

If you only need standard local notifications and standard remote-notification payloads, the plugin can still be minimal — for example, just setting `apsEnvMode` and Android icons.
