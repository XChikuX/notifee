...
Notifee currently does **not** ship an official Expo config plugin. If you use Expo with `expo prebuild`, the recommended approach is:

- use native `ios/` and `android/` project changes directly whenever possible,
- keep using `expo-build-properties` for SDK/build version changes, and
- only add a config plugin for build-time tasks that cannot be expressed at runtime.

In practice, the Expo-specific tasks that may still need plugin automation are:

- creating and maintaining an iOS Notification Service Extension,
- copying custom iOS notification sound files into the native targets, and
- generating or copying Android notification small-icon resources.

If you only need standard local notifications and standard remote-notification payloads, native project changes are usually enough after `expo prebuild`.
