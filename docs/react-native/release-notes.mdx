...
> Historical note: the `7.8.0` entry below refers to the removal of an older Expo integration. Current `@psync/notifee` releases ship an official Expo config plugin again. See [Installation](/react-native/docs/installation) for the current Expo setup.
