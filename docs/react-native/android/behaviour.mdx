...
If required, you can also provide a custom local sound resource or use a different device sound.

### Custom Sound

Custom sounds must be local files packaged into your Android app. Remote sound files are not supported here.

Android supports a wider range of audio containers than iOS notification sounds do. In practice, `.mp3`, `.wav`, and `.ogg` are the most predictable choices for Android notification sounds across devices.
...
If you are using Expo with `expo prebuild`, the `@psync/notifee` config plugin can copy these files for you with the `androidSoundFiles` option.
