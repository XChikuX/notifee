...
`smallIcon` must be the name of a native Android resource that already exists in `mipmap` or `drawable`. Runtime JS assets, `require(...)`, or Expo asset references are not valid values for `smallIcon`.

If you are using Expo, prefer keeping the generated `android/` project and copying/generating these resources during `expo prebuild`. A config plugin is only useful here for build-time resource generation or copying.
...
Unlike `smallIcon`, `largeIcon` can be provided from runtime assets or remote URLs, so most Expo apps do not need any special plugin support for `largeIcon`.
