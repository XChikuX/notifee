...
Starting with Android 12 (API level 31), the system restricts apps from starting foreground services while running in the background. If your app attempts to start a foreground service while in the background, the system will throw a `ForegroundServiceStartNotAllowedException`.

Notifee handles this gracefully by catching the exception and logging an actionable error message rather than crashing your app. However, the foreground service will not start in this case.

To avoid this restriction, ensure that your foreground service is started only when the app is in the foreground, or from an allowed exemption context such as:

- A high-priority Firebase Cloud Messaging (FCM) message
- A user interaction (e.g. tapping a notification action)
- An exact alarm callback
- A system broadcast that grants a temporary exemption

For more details, see the [Android foreground service background-start restrictions](https://developer.android.com/develop/background-work/services/fgs/restrictions-bg-start) documentation.