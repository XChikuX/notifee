...
If you are using Expo, there is currently no official Notifee config plugin that performs this setup for you. The preferred option is to keep the generated native iOS project and make these changes directly after `expo prebuild`. A config plugin is only useful here as build-time automation for the extension target, entitlements, and any sound-file copying.
