...
If you are using Expo, importing a sound through `expo-assets` or `expo-notifications` does not automatically make it available to `UNNotificationSound`. If you currently store notification sounds as `.mp3`, convert them to a supported notification-sound format first and then copy them into the native iOS target during `expo prebuild` or native project setup.
